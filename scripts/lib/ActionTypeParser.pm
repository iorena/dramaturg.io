...
sub generalize_action_type($action_type) {
    ...
    $action_type->{'subject'} = (exists $action_type->{'subject_is_propn'} ? Convert::person("3") : Convert::generalize_subject($action_type->{'subject'}));
    $action_type->{'verb'} = Convert::generalize_verb($action_type->{'verb'});
    $action_type->{'object'} = (exists $action_type->{'object_is_propn'} ? Convert::person("3") : Convert::generalize_object($action_type->{'object'}));
}
